...
" Définition de la mémoire occupée
"---------------------------------
set maxmem=8192
set maxmemtot=16384
...
if ( $VIM == substitute( $VIM, "^[A-Za-z]:", "", "" ) )
   ...
   "----------------------------------------
   let myVIMRep = "~/.VIMOUT" . localSubFolder
   let localFileDefinitions="~/.vimrclocal"
   ...
   "----------------------------------------
   let myVIMRep = "~/VIMOUT" . localSubFolder
   let localFileDefinitions="~/_vimrclocal"
   " Définition des commandes système
   "---------------------------------
...
if ( filereadable( expand( localFileDefinitions ) ) != 0 )
   execute "source " . localFileDefinitions
endif

if ( filereadable( expand( localFileDefinitions . localExtension ) ) != 0 )
   execute "source " . localFileDefinitions . localExtension
endif
