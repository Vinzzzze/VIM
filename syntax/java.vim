...
if exists("java_highlight_all")  || exists("java_highlight_java")  || exists("java_highlight_java_lang") 
  ...
  syn match javaR_JavaLang "\<\([_$a-zA-Z][_$a-zA-Z0-9]*\|\)Exception\>"
  ...
  syn keyword javaLangObject clone equals equalsIgnoreCase finalize getClass hashCode
  syn keyword javaLangObject notify notifyAll toString wait
  JavaHiLink javaLangObject		     javaConstant
  syn cluster javaTop add=javaLangObject

  syn keyword JavaStdFramework List ArrayList Set TreeSet HashMap Collection Arrays HashSet Iterator Map ByteArrayOutputStream
  syn keyword JavaStdFramework InputStream BufferedInputStream FilterInputStream File FileInputStream FileOutputStream
  syn keyword JavaStdFramework Calendar GregorianCalendar Date LocalDate LocalTime LocalDateTime LinkedHashMap GZIPInputStream GZIPOutputStream Locale
  syn keyword JavaStdFramework SimpleDateFormat Externalizable ObjectInput ObjectOutput OutputStream
  syn keyword javaError DateTime

  syn keyword JavaStdFrameworkException IOException FileNotFoundException UnknownHostException SocketException SocketTimeoutException
  syn cluster javaTop add=JavaStdFrameworkException
  syn cluster javaClasses add=JavaStdFrameworkException
  JavaHiLink javaR_Java JavaExceptionClasses
  JavaHiLink javaC_Java JavaExceptionClasses
  JavaHiLink javaE_Java JavaExceptionClasses
  JavaHiLink javaX_Java JavaExceptionClasses
  JavaHiLink JavaStdFrameworkException JavaExceptionClasses
endif
...
if !exists("java_ignore_javadoc") && main_syntax != 'jsp'
  ...
  if filereadable( expand( "%:p:h" ) . "/html.vim" )
...
if exists("java_highlight_mockito")
   syn keyword JavaTestComparer any anyDouble anyInt anyObject argThat eq exists overridingErrorMessage onProperty

   syn keyword JavaTestResult assertThat Assert isCloseTo within isEqualTo isTrue isFalse notNull isNotNull contentOf
   syn keyword JavaTestResult isGreaterThan isEmpty isNotEmpty isInstanceOf isNotEqualTo \contains hasSize isNullOrEmpty
   syn keyword JavaTestResult describedAs containsIgnoringCase isNull isLessThan isLessThanOrEqualTo isGreaterThan containsAll
   syn keyword JavaTestResult isFile containsOnly containsExactly isGreaterThanOrEqualTo isEqualToIgnoringGivenFields hasSameSizeAs

   syn keyword JavaMockitoDefinition spy mock when Answer Invocation ArgumentCaptor ArgumentMatcher withSetting stub stubVoid
   syn keyword JavaMockitoAction doReturn doNothing doAnswer doThrow clear reset fail thenReturn then thenThrow
   syn keyword JavaMockitoVerification atLeast atLeastOnce atMost never only timeout times verify isWaitingAtExactly
   syn keyword JavaMockitoVerification verifyNoMoreInteractions verifyZeroInteractions inOrder isWaitingAt isStarted
endif

if exists("java_parenthesis_error")
...
if version >= 508 || !exists("did_java_syn_inits")
  ...
  highlight JavaStdFramework guifg=Yellow guibg=Black gui=italic
  highlight JavaExceptionClasses guifg=Red guibg=Black gui=italic
  highlight JavaTestComparer guifg=White guibg=Black
  highlight JavaTestResult guifg=Red guibg=DarkBlue gui=bold
  highlight JavaMockitoDefinition guifg=White guibg=DarkGreen
  highlight JavaMockitoAction guifg=Yellow guibg=DarkGreen
  highlight JavaMockitoVerification guifg=White guibg=DarkRed
endif
