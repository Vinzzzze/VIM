...
"-------------------------------------
let s:className = input( "File base : " )
echo "\n"
...
if ( input( "Valide ( Y/N ) ? " ) == "Y" )
   ...
   "---------------------------------------
   call VM_ProjectBuildFile( s:commandUsed, s:className, "cfct", s:comment, s:className . ".cpp", s:repository, s:projectFolder )

   " Construit le fichier d'entête de class
   "---------------------------------------
   call VM_ProjectBuildFile( "vsplit", s:className, "hfct", s:comment, s:className . ".hpp", s:repository, s:projectFolder )
endif
